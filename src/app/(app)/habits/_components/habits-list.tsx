...
import { MoreVertical, Pause, Play, Archive, Trash2, Target, Plus } from "lucide-react";
...
export function HabitsList() {
  ...
  const archivedHabits = habits.filter((h) => h.status === "ARCHIVED" || h.status === "COMPLETED");

  return (
    <Tabs defaultValue="active" className="space-y-4">
      <TabsList>
        <TabsTrigger value="active">
          进行中 ({activeHabits.length})
        </TabsTrigger>
        <TabsTrigger value="paused">
          已暂停 ({pausedHabits.length})
        </TabsTrigger>
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {habit.status === "ACTIVE" ? (
                <DropdownMenuItem onClick={() => pauseMutation.mutate({ id: habit.id })}>
                  <Pause className="mr-2 h-4 w-4" />
                  暂停
                </DropdownMenuItem>
              ) : habit.status === "PAUSED" ? (
                <DropdownMenuItem onClick={() => resumeMutation.mutate({ id: habit.id })}>
                  <Play className="mr-2 h-4 w-4" />
                  恢复
                </DropdownMenuItem>
              ) : null}
              <DropdownMenuItem onClick={() => archiveMutation.mutate({ id: habit.id })}>
              ...
              <DropdownMenuItem
                ...
                onClick={() => {
                  if (confirm("确定要删除这个习惯吗？所有相关数据将被永久删除。")) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">第 {habit.currentPhase} 阶段</span>
            <span className="text-muted-foreground">累计 {habit._count.logs} 天</span>
          </div>
          <Progress value={(habit.currentPhase / 5) * 100} className="h-2" />
        </div>

        <Button variant="outline" className="w-full" asChild>
          <Link href={`/habits/${habit.id}`}>查看详情</Link>
        </Button>
      </CardContent>
    </Card>
  );
