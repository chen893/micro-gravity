...
export function StreakChart({
  ...
  const chartData = data.map((item) => ({
    name: item.habitName.length > 6 ? item.habitName.slice(0, 6) + "..." : item.habitName,
    fullName: item.habitName,
    current: item.currentStreak,
    longest: item.longestStreak,
  }));

  const maxStreak = Math.max(...data.map((d) => Math.max(d.currentStreak, d.longestStreak)), 7);
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex h-64 items-center justify-center">
            <p className="text-sm text-muted-foreground">暂无连续数据</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      ...
      <CardContent>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              <Bar dataKey="longest" fill="hsl(var(--chart-2))" radius={[0, 4, 4, 0]} barSize={20}>
                {chartData.map((_, index) => (
                  <Cell key={`cell-longest-${index}`} fillOpacity={0.3} />
                ))}
              </Bar>
              <Bar dataKey="current" fill="hsl(var(--chart-1))" radius={[0, 4, 4, 0]} barSize={20}>
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-current-${index}`}
                    fill={entry.current >= 7 ? "hsl(var(--chart-1))" : "hsl(var(--chart-3))"}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <div className="flex items-center justify-center gap-4 mt-2">
          ...
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            <span className="h-2 w-2 rounded-full bg-[hsl(var(--chart-2))]" style={{ opacity: 0.3 }} />
            历史最长
          </div>
        </div>
      </CardContent>
