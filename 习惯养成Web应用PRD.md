  # 习惯养成 Web 应用产品需求文档（PRD）

  **基于福格行为模型与 AI SDK v6 的智能习惯管理系统**

  ---

  | 文档信息 | |
  |---------|---|
  | 版本 | v2.4 |
  | 日期 | 2025-11-28 |
  | 状态 | AI SDK v6 升级版 |
  | 技术框架 | T3 Stack + Vercel AI SDK v6 |

  ---

  ## 目录

  1. [概述](#1-概述)
  2. [福格行为模型与产品映射](#2-福格行为模型与产品映射)
  3. [系统架构设计](#3-系统架构设计)
  4. [核心功能模块](#4-核心功能模块)
     - 4.1 AI 对话式习惯配置
     - 4.2 智能提醒系统
     - 4.3 动机维护引擎
     - 4.4 能力评估与任务拆解
     - 4.5 坏习惯戒除系统
     - 4.6 AI 教练对话
     - 4.7 数据洞察
     - 4.8 周期报告系统 (v1.5) ⭐
     - 4.9 进阶分析系统 (v1.5) ⭐
  5. [AI SDK 技术实现](#5-ai-sdk-技术实现)
  6. [数据库设计](#6-数据库设计)
  7. [非功能需求](#7-非功能需求)
  8. [版本迭代规划](#8-版本迭代规划)

  ---

  ## 1. 概述

  ### 1.1 背景与目标

  #### 市场痛点

  | 痛点 | 现有产品问题 | 本产品解决方案 |
  |-----|------------|--------------|
  | 动机衰减 | 简单打卡，无持续激励 | AI 动态感知动机状态，个性化激励 |
  | 能力错配 | 固定目标，不考虑个体差异 | 智能任务拆解，微习惯渐进设计 |
  | 提示失效 | 机械定时推送 | 情境感知，最佳时机预测 |
  | 戒除困难 | 缺乏科学方法论支撑 | 基于福格模型的触发链打断策略 |

  #### 产品目标

  构建基于 **福格行为模型（B=MAP）** 的智能习惯管理系统：

  - **M（Motivation）**：AI 驱动的动机诊断与个性化激励
  - **A（Ability）**：智能任务拆解与难度动态调整
  - **P（Prompt）**：情境感知的智能触发系统

  ### 1.2 目标用户

  | 用户群体 | 核心场景 |
  |---------|---------|
  | 职场人群 | 早起、阅读、时间管理 |
  | 健康追求者 | 运动、冥想、健康饮食 |
  | 戒除需求者 | 戒烟、减少手机、控制消费 |
  | 自我提升者 | 学习技能、写作、专注力 |

  ### 1.3 技术选型

  ```
  Frontend
  ├── Next.js 15 (App Router + Turbopack)
  ├── React 19 + TypeScript
  ├── Tailwind CSS v4 + shadcn/ui
  ├── @ai-sdk/react (useChat hook)
  └── Recharts (数据可视化)

  Backend
  ├── tRPC v11 (类型安全API)
  ├── Vercel AI SDK v6 (ai package)
  ├── Prisma ORM
  ├── PostgreSQL
  ├── NextAuth.js v5 (认证)
  └── Zod (Schema验证)

  Infrastructure
  ├── Vercel (部署)
  ├── Vercel AI Gateway (统一模型访问)
  └── PostgreSQL (数据库)
  ```

  ---

  ## 2. 福格行为模型与产品映射

  ### 2.1 核心公式

  ```
  B = MAP

  行为发生 = 动机(Motivation) × 能力(Ability) × 提示(Prompt)
  ```

  ### 2.2 行为设计曲线

  ```
  动机 ↑
    │      ★ 行为发生区
    │    ╱─────────────
    │  ╱ 行动线
    │╱
    ├─────────────────→ 能力（简单程度）

  条件：当 动机×能力 ≥ 行动阈值 且 存在有效提示 → 行为发生
  ```

  ### 2.3 产品功能映射

  | 福格要素 | 理论核心 | AI 功能实现 |
  |---------|---------|-----------|
  | **动机(M)** | 愉悦/痛苦、希望/恐惧、社会认同 | 动机类型诊断、个性化激励文案生成、愿景可视化 |
  | **能力(A)** | 时间、金钱、体力、脑力、社会偏差、非常规性 | 微习惯设计、智能任务拆解、难度动态调整 |
  | **提示(P)** | 信号型、便利型、火花型 | 最佳时机预测、锚定习惯设计、情境化提醒生成 |

  ### 2.4 养成 vs 戒除策略对比

  | 维度 | 习惯养成 | 坏习惯戒除 |
  |-----|---------|-----------|
  | 动机策略 | 强化正向愿景 | 强化痛苦感知 |
  | 能力策略 | 降低行动门槛 | 提高执行障碍 |
  | 提示策略 | 创造新触发点 | 打断触发链 |

  ---

  ## 3. 系统架构设计

  ### 3.1 整体架构

  ```
  ┌─────────────────────────────────────────────────────────────────┐
  │                      Frontend (Next.js)                         │
  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
  │  │  Chat UI     │  │  Dashboard   │  │  Progress    │          │
  │  │  (useChat)   │  │  (习惯管理)   │  │  (数据可视化) │          │
  │  └──────────────┘  └──────────────┘  └──────────────┘          │
  └─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
  ┌─────────────────────────────────────────────────────────────────┐
  │                   API Layer (tRPC v11)                          │
  │  ┌──────────────────────────────────────────────────────────┐  │
  │  │  chatRouter       - AI对话端点 (streamText)              │  │
  │  │  habitRouter      - 习惯CRUD                             │  │
  │  │  logRouter        - 打卡记录                              │  │
  │  │  insightRouter    - 智能洞察 (generateObject)            │  │
  │  └──────────────────────────────────────────────────────────┘  │
  └─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
  ┌─────────────────────────────────────────────────────────────────┐
  │                    AI SDK Core Layer                            │
  │  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐    │
  │  │   streamText   │  │ generateObject │  │     tool()     │    │
  │  │   (对话流式)    │  │  (结构化输出)   │  │   (工具调用)    │    │
  │  └────────────────┘  └────────────────┘  └────────────────┘    │
  │                              │                                  │
  │                    ┌─────────┴─────────┐                       │
  │                    │ Vercel AI Gateway │                       │
  │                    │  (统一模型访问)    │                       │
  │                    └───────────────────┘                       │
  └─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
  ┌─────────────────────────────────────────────────────────────────┐
  │                     Data Layer                                  │
  │  ┌──────────────────────┐  ┌──────────────────────┐            │
  │  │     PostgreSQL       │  │    Prisma ORM        │            │
  │  │                      │  │         │            │
  │  └──────────────────────┘  └──────────────────────┘            │
  └─────────────────────────────────────────────────────────────────┘
  ```

  ### 3.2 模型配置（Vercel AI Gateway）

  ```env
  # .env.local
  AI_GATEWAY_API_KEY=your-api-key
  ```

  ```typescript
  // 直接在调用时指定模型，无需单独配置文件
  // 支持的模型格式：provider/model-name
  model: 'openai/gpt-4o'
  model: 'anthropic/claude-sonnet-4-20250514'
  model: 'google/gemini-2.5-flash'
  ```

  > **设计说明**：使用 Vercel AI Gateway 统一管理模型访问，通过单一 API Key 访问多个提供商的模型。在代码中直接指定模型字符串，便于灵活切换。

  ---

  ## 4. 核心功能模块

  ### 4.1 模块一：AI 对话式习惯配置

  #### 功能概述

  通过 AI 对话引导用户完成习惯创建，替代传统表单方式，同时深入挖掘用户动机。

  #### 对话流程

  ```
  Step 1: 意图采集
  ┌─────────────────────────────────────────────┐
  │ AI: "你好！想养成新习惯，还是改掉一个坏习惯？"  │
  │ 用户: "我想每天早起"                          │
  └─────────────────────────────────────────────┘
                      ↓
  Step 2: 动机深挖 (MAP-M)
  ┌─────────────────────────────────────────────┐
  │ AI: "早起对你意味着什么？是想有更多自己的      │
  │      时间，还是为了工作效率？"                 │
  │ 用户: "想有时间看书学习"                      │
  └─────────────────────────────────────────────┘
                      ↓
  Step 3: 能力评估 (MAP-A)
  ┌─────────────────────────────────────────────┐
  │ AI: "你现在通常几点起床？目标是几点？"         │
  │ 用户: "现在8点，想6点起"                      │
  │ AI: "一下提前2小时挑战较大。建议从7:30开始，   │
  │      每周提前15分钟，你觉得呢？"               │
  └─────────────────────────────────────────────┘
                      ↓
  Step 4: 触发设计 (MAP-P)
  ┌─────────────────────────────────────────────┐
  │ AI: "晚上睡前你通常做什么？我们可以把          │
  │      '准备早起'锚定到你的睡前习惯上"           │
  │ 用户: "会刷一会手机"                          │
  │ AI: "那我们设定：当你拿起手机准备刷时，        │
  │      先把明天的衣服放在床边。这只需30秒。"     │
  └─────────────────────────────────────────────┘
  ```

  #### 技术实现

  ```typescript
  // src/app/api/chat/route.ts
  import { streamText, convertToModelMessages, tool, UIMessage } from 'ai';
  import { z } from 'zod';
  import { db } from '@/server/db';
  import { auth } from '@/server/auth';

  export async function POST(req: Request) {
    const session = await auth();
    if (!session?.user) {
      return new Response('Unauthorized', { status: 401 });
    }

    const { messages }: { messages: UIMessage[] } = await req.json();

    const result = streamText({
      model: 'openai/gpt-4o',
      system: HABIT_COACH_SYSTEM_PROMPT,
      messages: convertToModelMessages(messages),
      tools: {
        createHabit: tool({
          description: '当收集完习惯信息后，创建习惯配置',
          inputSchema: z.object({
            name: z.string().describe('习惯名称'),
            type: z.enum(['BUILD', 'BREAK']).describe('养成或戒除'),
            category: z.string().optional().describe('习惯分类'),
            motivation: z.object({
              primaryType: z.enum(['PLEASURE', 'HOPE', 'SOCIAL']).describe('主要动机类型'),
              deepReason: z.string().describe('深层原因'),
              visionStatement: z.string().describe('愿景声明'),
              motivationScore: z.number().min(1).max(10).describe('当前动机强度'),
            }),
            ability: z.object({
              currentLevel: z.string().describe('当前水平'),
              targetLevel: z.string().describe('目标水平'),
              microHabit: z.string().describe('微习惯定义'),
              difficultyScore: z.number().min(1).max(10).describe('预估难度'),
            }),
            prompt: z.object({
              anchorHabit: z.string().describe('锚定习惯'),
              triggerType: z.enum(['SIGNAL', 'FACILITATOR', 'SPARK']).describe('触发类型'),
              preferredTime: z.string().describe('偏好时间'),
            }),
          }),
          execute: async (habitConfig) => {
            // 使用 Prisma 创建习惯
            return db.habit.create({
              data: {
                ...habitConfig,
                userId: session.user.id,
              },
            });
          },
        }),
      },
    });

    return result.toUIMessageStreamResponse();
  }
  ```

  > **注意**: AI 流式响应需要配合 Next.js API Route 使用，tRPC 主要处理习惯 CRUD 等标准操作。对于 AI 对话，推荐使用混合模式：`/api/chat` 处理流式 AI 响应，tRPC 处理数据操作。

  ### 4.2 模块二：智能提醒系统

  #### 提醒策略矩阵

  ```
                      动机水平
                  低        高
                ┌────────┬────────┐
          低   │ 火花型  │ 信号型  │
    能力        │ (激励+指引) │ (简单提醒) │
    门槛        ├────────┼────────┤
          高   │ 便利型  │ 信号型  │
                │ (降低门槛) │ (简单提醒) │
                └────────┴────────┘
  ```

  #### 智能文案生成

  ```typescript
  // lib/ai/reminder.ts
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const reminderSchema = z.object({
    content: z.string().describe('提醒文案，30-60字'),
    promptType: z.enum(['SIGNAL', 'FACILITATOR', 'SPARK']),
    actionGuidance: z.string().describe('具体行动指引'),
  });

  export async function generateReminder(context: ReminderContext) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `你是习惯教练，精通福格行为模型。
  根据用户当前状态生成个性化提醒。
  - 动机类型: ${context.motivationType}
  - 当前动机水平: ${context.motivationLevel}/10
  - 连续完成天数: ${context.streakDays}
  - 最近完成率: ${context.recentRate}%`,
      prompt: `为习惯"${context.habitName}"生成提醒文案`,
      schema: reminderSchema,
    });

    return object;
  }
  ```

  #### 示例输出对比

  | 场景 | 传统提醒 | AI 智能提醒 |
  |-----|---------|-----------|
  | 早起-动机低 | "该起床了！" | "小李，昨天你说想在晋升前读完那本书。今天早起的30分钟，就是你领先同龄人的时刻。" |
  | 运动-能力不足 | "运动时间到！" | "今天不用去健身房。穿上鞋，在小区走10分钟就算完成。下雨？那就客厅5个深蹲。" |
  | 戒烟-触发时刻 | "别抽烟！" | "检测到休息时间。深呼吸3次，喝一口水，给自己1分钟。你已经连续3天成功度过这个时刻了。" |

  ### 4.3 模块三：动机维护引擎

  #### 动机波动模型

  ```
  动机强度
      │
  10 ─┤     ╭─╮        ╭─╮
      │    ╱   ╲      ╱   ╲    ← AI激励干预
  7 ─┤   ╱     ╲    ╱     ╲
      │  ╱       ╲  ╱       ╲
  4 ─┤ ╱         ╲╱         ╲  ← 自然衰减
      │╱                       ╲
  1 ─┼───────────────────────────→ 时间
      Day1    Day7   Day14   Day21

  关键干预点:
  ├── Day 3: 新鲜感消退
  ├── Day 7: 第一个里程碑
  ├── Day 14: 倦怠高发期
  └── Day 21+: 习惯初步形成
  ```

  #### 激励内容生成

  ```typescript
  // lib/ai/motivation.ts
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const motivationSchema = z.object({
    message: z.string().describe('激励文案，50-100字'),
    motivationType: z.enum([
      'PLEASURE_GAIN',   // 获得愉悦
      'PAIN_AVOID',      // 规避痛苦
      'HOPE_FUTURE',     // 希望愿景
      'FEAR_LOSS',       // 损失恐惧
      'SOCIAL_PROOF',    // 社会认同
    ]),
    actionSuggestion: z.string().describe('下一步行动建议'),
  });

  export async function generateMotivation(context: MotivationContext) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `你是习惯教练。根据用户的动机类型和当前状态，
  生成能引发共鸣的激励内容。
  用户深层动机: ${context.deepReason}
  用户愿景: ${context.visionStatement}`,
      prompt: `用户已连续${context.streakDays}天完成"${context.habitName}"，
  当前状态: ${context.currentState}
  请生成个性化激励`,
      schema: motivationSchema,
    });

    return object;
  }
  ```

  ### 4.4 模块四：能力评估与任务拆解

  #### 六维能力评估（福格模型）

  ```
          时间
          │
    ┌─────┼─────┐
    │     │     │
  金钱─────┼─────体力
    │     │     │
    └─────┼─────┘
          │
    ┌─────┼─────┐
    │     │     │
  脑力─────┼─────社会偏差
    │     │     │
    └─────┼─────┘
          │
        非常规性
  ```

  #### 智能任务拆解

  ```typescript
  // lib/ai/ability.ts
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const phaseSchema = z.object({
    phase: z.number(),
    duration: z.string().describe('建议持续时间'),
    microHabit: z.string().describe('微习惯定义'),
    successCriteria: z.string().describe('成功标准'),
    difficultyScore: z.number().min(1).max(10),
  });

  const taskBreakdownSchema = z.object({
    analysis: z.string().describe('当前难度分析'),
    phases: z.array(phaseSchema).describe('渐进式阶段'),
    tips: z.array(z.string()).describe('简化建议'),
  });

  export async function breakdownHabit(habit: HabitConfig) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `你是习惯设计专家，精通"2分钟规则"和微习惯理论。
  将用户的目标习惯拆解为渐进式阶段，每个阶段应该足够简单，
  让用户"不可能失败"。`,
      prompt: `目标习惯: ${habit.targetLevel}
  当前状态: ${habit.currentLevel}
  请设计渐进式计划`,
      schema: taskBreakdownSchema,
    });

    return object;
  }
  ```

  **拆解示例：每天运动1小时**

  ```
  阶段1 (第1-7天): 运动装备就位
  └── 微习惯：每天把运动鞋放在门口（30秒）
  └── 成功标准：鞋在门口就算完成

  阶段2 (第2周): 穿上装备
  └── 微习惯：穿上运动服和鞋（2分钟）
  └── 成功标准：穿上就算完成

  阶段3 (第3周): 走出家门
  └── 微习惯：穿好装备走出家门（5分钟）
  └── 成功标准：出门即完成

  阶段4 (第4-5周): 10分钟活动
  └── 任务：任意运动10分钟

  阶段5 (第6-8周): 30分钟运动

  阶段6 (第9周+): 1小时运动 ✓
  ```

  ### 4.5 模块五：坏习惯戒除系统

  #### 习惯环路打断

  ```
  传统习惯环路:
  ┌─────────┐
  │  触发点  │ ←── AI识别触发模式
  └────┬────┘
      ↓
  ┌─────────┐
  │  渴望   │ ←── 分析深层需求
  └────┬────┘
      ↓
  ┌─────────┐
  │  响应   │ ←── 设计替代行为
  └────┬────┘
      ↓
  ┌─────────┐
  │  奖励   │ ←── 重新定义奖励
  └─────────┘
  ```

  #### 触发模式分析

  ```typescript
  // lib/ai/break-habit.ts
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const triggerAnalysisSchema = z.object({
    patterns: z.array(z.object({
      type: z.enum(['TEMPORAL', 'CONTEXTUAL', 'EMOTIONAL', 'BEHAVIORAL']),
      description: z.string(),
      confidence: z.number().min(0).max(1),
      evidence: z.array(z.string()),
    })),
    deepNeed: z.string().describe('坏习惯满足的深层需求'),
    substituteBehaviors: z.array(z.string()).describe('替代行为建议'),
    environmentDesign: z.array(z.string()).describe('环境设计建议'),
  });

  export async function analyzeTriggers(records: TriggerRecord[]) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `你是行为分析专家，擅长识别习惯触发模式。
  分析框架：
  1. 时间模式：什么时间容易触发
  2. 情境模式：什么场景容易触发
  3. 情绪模式：什么情绪状态容易触发
  4. 前序行为：触发前通常在做什么`,
      prompt: `分析以下触发记录：
  ${JSON.stringify(records, null, 2)}`,
      schema: triggerAnalysisSchema,
    });

    return object;
  }
  ```

  #### 复发管理

  ```
  复发响应流程:

  用户记录复发 → AI共情响应 → 引导分析触发点 → 模式分析 → 策略迭代 → 重建信心
      │              │              │            │           │          │
      ↓              ↓              ↓            ↓           ↓          ↓
    "我又抽烟了"   "复发是正常的，  "当时什么情况？   识别新模式   调整策略    "你已连续X天
                  我们来看看发生    你的感受是什么？"             建议       成功，这次只是
                  了什么"                                                一个数据点"
  ```

  ### 4.6 模块六：AI 教练对话

  #### 对话场景

  | 场景类型 | 描述 | 示例 |
  |---------|------|-----|
  | 习惯设定 | 创建新习惯、调整目标 | "我想养成早起的习惯" |
  | 日常签到 | 完成确认、困难反馈 | "今天完成了！" |
  | 困难求助 | 遇到障碍、寻求建议 | "最近总是坚持不下去" |
  | 复盘分析 | 周期回顾、策略优化 | "帮我分析下这周的情况" |

  #### 前端实现

  ```tsx
  // src/app/_components/habit-coach.tsx
  'use client';

  import { useChat } from '@ai-sdk/react';
  import { DefaultChatTransport } from 'ai';
  import { useState } from 'react';
  import { useSession } from 'next-auth/react';

  export function HabitCoach() {
    const [input, setInput] = useState('');
    const { data: session } = useSession();

    const { messages, sendMessage, isLoading } = useChat({
      transport: new DefaultChatTransport({
        api: '/api/chat',
      }),
    });

    if (!session) {
      return <div>请先登录</div>;
    }

    return (
      <div className="flex flex-col w-full max-w-2xl mx-auto p-4">
        <div className="flex-1 space-y-4 mb-4">
          {messages.map(m => (
            <div key={m.id} className="whitespace-pre-wrap">
              <div className="font-bold">
                {m.role === 'user' ? '你' : '习惯教练'}
              </div>
              {m.parts.map((part, index) => {
                if (part.type === 'text') {
                  return <p key={index}>{part.text}</p>;
                }
                if (part.type === 'tool-invocation' && part.toolName === 'createHabit') {
                  return (
                    <div key={index} className="bg-green-50 p-3 rounded-lg">
                      ✅ 习惯创建成功！
                    </div>
                  );
                }
                return null;
              })}
            </div>
          ))}
        </div>

        <form
          onSubmit={e => {
            e.preventDefault();
            sendMessage({ text: input });
            setInput('');
          }}
          className="flex gap-2"
        >
          <input
            value={input}
            onChange={e => setInput(e.target.value)}
            placeholder="和习惯教练聊聊..."
            className="flex-1 p-3 border rounded-lg"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading}
            className="px-6 py-3 bg-blue-600 text-white rounded-lg"
          >
            发送
          </button>
        </form>
      </div>
    );
  }
  ```

  > **说明**: 使用 `useSession` 检查用户登录状态，AI 对话需要用户已登录。tRPC 用于习惯数据的 CRUD 操作。

  #### 系统提示词

  ```typescript
  // lib/ai/prompts.ts
  export const HABIT_COACH_SYSTEM_PROMPT = `
  你是一位专业的习惯教练，精通福格行为模型（Fogg Behavior Model）。

  ## 核心方法论
  基于 B=MAP 公式帮助用户：
  - M(动机)：帮助用户发现和维护内在动机
  - A(能力)：帮助用户降低行动门槛，设计微习惯
  - P(提示)：帮助用户设计有效的触发机制

  ## 对话原则
  1. 每次只问一个问题，循序渐进
  2. 先倾听，理解用户真实需求
  3. 给出可执行的小步骤，而非大道理
  4. 用提问引导用户自己发现答案
  5. 庆祝每一个小进步，对失败不评判

  ## 风格
  - 温和但直接
  - 支持但不迁就
  - 专业但不说教
  - 简洁，每次回复不超过100字
  `;
  ```

  ### 4.7 模块七：数据洞察

  #### 洞察生成

  ```typescript
  // lib/ai/insights.ts
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const insightSchema = z.object({
    positive: z.object({
      title: z.string(),
      content: z.string().max(50),
    }).describe('正向强化洞察'),
    pattern: z.object({
      title: z.string(),
      content: z.string().max(50),
    }).describe('模式识别洞察'),
    suggestion: z.object({
      title: z.string(),
      content: z.string().max(50),
      action: z.string().describe('具体行动建议'),
    }).describe('优化建议洞察'),
  });

  export async function generateInsights(data: HabitData) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `你是数据分析师，擅长从习惯数据中发现模式和洞察。
  生成3条简洁有力的洞察，每条不超过50字。`,
      prompt: `习惯数据：
  ${JSON.stringify(data, null, 2)}`,
      schema: insightSchema,
    });

    return object;
  }
  ```

  #### 洞察示例

  ```
  ✨ 正向强化
  "冥想连续7天全勤！这是你坚持最久的习惯。"

  🔍 模式识别
  "周四运动容易中断——和加班日重叠，这不是意志力问题。"

  💡 优化建议
  "试试把周四的运动改为5分钟拉伸。保持连贯比强度更重要。"
  ```

  ### 4.8 模块八：周期报告系统 (v1.5)

  #### 功能概述

  自动生成周报/月报，帮助用户回顾习惯养成进展，提供深度分析和下阶段建议。

  #### 报告类型

  | 报告类型 | 生成周期 | 核心内容 |
  |---------|---------|---------|
  | 周报 | 每周日晚 | 本周完成率、亮点习惯、改进建议 |
  | 月报 | 每月最后一天 | 月度趋势、里程碑回顾、下月规划 |
  | 阶段报告 | 习惯达到21/66天 | 阶段成就、习惯强度评估、升级建议 |

  #### 报告结构设计

  ```
  ┌─────────────────────────────────────────────────────────────┐
  │                    📊 习惯周报                               │
  │                  2025.11.20 - 11.27                         │
  ├─────────────────────────────────────────────────────────────┤
  │  📈 总体概览                                                 │
  │  ┌─────────────────────────────────────────────────────┐   │
  │  │  本周完成率: 85% (↑12%)  │  活跃习惯: 4个           │   │
  │  │  最长连续: 14天          │  累计打卡: 24次          │   │
  │  └─────────────────────────────────────────────────────┘   │
  ├─────────────────────────────────────────────────────────────┤
  │  🏆 本周亮点                                                 │
  │  • 早起习惯连续7天全勤，已进入第二阶段！                      │
  │  • 阅读时间较上周增加40分钟                                  │
  ├─────────────────────────────────────────────────────────────┤
  │  🔍 模式发现                                                 │
  │  • 周三完成率最低(60%)，可能与周中疲劳相关                   │
  │  • 早晨习惯完成率(92%)显著高于晚间(68%)                     │
  ├─────────────────────────────────────────────────────────────┤
  │  💡 下周建议                                                 │
  │  • 考虑将"运动"从晚间调整到早晨                              │
  │  • 周三可尝试执行简化版微习惯                                 │
  ├─────────────────────────────────────────────────────────────┤
  │  🎯 下周目标                                                 │
  │  □ 早起连续达到14天                                          │
  │  □ 运动完成率提升至80%                                       │
  └─────────────────────────────────────────────────────────────┘
  ```

  #### 技术实现

  ```typescript
  // src/server/api/routers/report.ts
  import { createTRPCRouter, protectedProcedure } from '@/server/api/trpc';
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const weeklyReportSchema = z.object({
    summary: z.object({
      completionRate: z.number(),
      rateChange: z.number(),
      activeHabits: z.number(),
      longestStreak: z.number(),
      totalCheckins: z.number(),
    }),
    highlights: z.array(z.object({
      habitName: z.string(),
      achievement: z.string(),
      emoji: z.string(),
    })),
    patterns: z.array(z.object({
      finding: z.string(),
      implication: z.string(),
      confidence: z.number().min(0).max(1),
    })),
    suggestions: z.array(z.object({
      category: z.enum(['TIMING', 'DIFFICULTY', 'MOTIVATION', 'TRIGGER']),
      suggestion: z.string(),
      expectedImpact: z.string(),
    })),
    nextWeekGoals: z.array(z.object({
      goal: z.string(),
      habitId: z.string().optional(),
      measurable: z.string(),
    })),
  });

  export const reportRouter = createTRPCRouter({
    // 获取周报
    getWeeklyReport: protectedProcedure
      .input(z.object({
        weekStart: z.date(),
      }))
      .query(async ({ ctx, input }) => {
        // 获取本周数据
        const weekData = await ctx.db.habitLog.findMany({
          where: {
            userId: ctx.session.user.id,
            loggedAt: {
              gte: input.weekStart,
              lt: new Date(input.weekStart.getTime() + 7 * 24 * 60 * 60 * 1000),
            },
          },
          include: { habit: true },
        });

        const habits = await ctx.db.habit.findMany({
          where: { userId: ctx.session.user.id, status: 'ACTIVE' },
        });

        // AI 生成报告
        const { object: report } = await generateObject({
          model: 'openai/gpt-4o',
          system: `你是习惯数据分析师，擅长从数据中发现有价值的洞察。
生成周报时：
1. 突出正向成就，给予肯定
2. 发现数据中的规律和模式
3. 提供具体可行的改进建议
4. 设定合理的下周目标`,
          prompt: `基于以下数据生成周报：
习惯列表: ${JSON.stringify(habits)}
本周记录: ${JSON.stringify(weekData)}`,
          schema: weeklyReportSchema,
        });

        return report;
      }),

    // 获取月报
    getMonthlyReport: protectedProcedure
      .input(z.object({
        year: z.number(),
        month: z.number(),
      }))
      .query(async ({ ctx, input }) => {
        // 实现类似，聚合月度数据
        // ...
      }),

    // 生成阶段性里程碑报告
    getMilestoneReport: protectedProcedure
      .input(z.object({
        habitId: z.string(),
        milestone: z.enum(['DAY_7', 'DAY_21', 'DAY_66']),
      }))
      .query(async ({ ctx, input }) => {
        // 实现里程碑报告生成
        // ...
      }),
  });
  ```

  #### 报告推送策略

  ```typescript
  // lib/jobs/report-scheduler.ts
  // 使用 Vercel Cron Jobs 定时生成报告

  // vercel.json
  {
    "crons": [
      {
        "path": "/api/cron/weekly-report",
        "schedule": "0 20 * * 0"  // 每周日晚8点
      },
      {
        "path": "/api/cron/monthly-report",
        "schedule": "0 20 L * *"  // 每月最后一天晚8点
      }
    ]
  }
  ```

  ### 4.9 模块九：进阶分析系统 (v1.5)

  #### 功能概述

  提供深度数据分析能力，帮助用户理解习惯养成的深层规律。

  #### 分析维度

  ```
  ┌─────────────────────────────────────────────────────────────────┐
  │                      进阶分析仪表板                              │
  ├─────────────────────────────────────────────────────────────────┤
  │                                                                 │
  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
  │  │   时间热力图     │  │   情绪关联分析   │  │   难度曲线图     │ │
  │  │   (完成时段)     │  │   (心情变化)     │  │   (主观感受)     │ │
  │  └─────────────────┘  └─────────────────┘  └─────────────────┘ │
  │                                                                 │
  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
  │  │   习惯关联矩阵   │  │   连续性分析     │  │   预测模型       │ │
  │  │   (相互影响)     │  │   (中断风险)     │  │   (成功概率)     │ │
  │  └─────────────────┘  └─────────────────┘  └─────────────────┘ │
  │                                                                 │
  └─────────────────────────────────────────────────────────────────┘
  ```

  #### 核心分析功能

  **1. 时间热力图分析**

  ```typescript
  // lib/ai/analytics.ts
  const timeHeatmapSchema = z.object({
    heatmap: z.array(z.object({
      dayOfWeek: z.number().min(0).max(6),
      hourOfDay: z.number().min(0).max(23),
      completionRate: z.number(),
      avgDuration: z.number().optional(),
    })),
    insights: z.array(z.string()),
    optimalWindows: z.array(z.object({
      dayOfWeek: z.number(),
      startHour: z.number(),
      endHour: z.number(),
      reason: z.string(),
    })),
  });

  export async function analyzeTimePatterns(logs: HabitLog[]) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `分析习惯完成的时间模式，找出最佳执行时间窗口。`,
      prompt: `习惯记录数据: ${JSON.stringify(logs)}`,
      schema: timeHeatmapSchema,
    });
    return object;
  }
  ```

  **2. 情绪关联分析**

  ```typescript
  const moodCorrelationSchema = z.object({
    beforeAfterCorrelation: z.object({
      averageMoodBefore: z.number(),
      averageMoodAfter: z.number(),
      moodLift: z.number(),
      significance: z.enum(['HIGH', 'MEDIUM', 'LOW']),
    }),
    moodTriggers: z.array(z.object({
      trigger: z.string(),
      impactOnCompletion: z.number(), // -1 到 1
      frequency: z.number(),
    })),
    recommendations: z.array(z.string()),
  });

  export async function analyzeMoodCorrelation(logs: HabitLog[]) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `分析情绪与习惯完成的关联性。
重点关注：
1. 习惯前后情绪变化
2. 低情绪状态下的完成策略
3. 习惯对情绪的正向影响`,
      prompt: `记录数据: ${JSON.stringify(logs)}`,
      schema: moodCorrelationSchema,
    });
    return object;
  }
  ```

  **3. 习惯关联矩阵**

  ```typescript
  const habitCorrelationSchema = z.object({
    correlations: z.array(z.object({
      habit1Id: z.string(),
      habit2Id: z.string(),
      correlationScore: z.number().min(-1).max(1),
      relationship: z.enum(['POSITIVE', 'NEGATIVE', 'NEUTRAL']),
      insight: z.string(),
    })),
    clusters: z.array(z.object({
      name: z.string(),
      habitIds: z.array(z.string()),
      description: z.string(),
    })),
    suggestions: z.array(z.object({
      type: z.enum(['STACK', 'SEPARATE', 'SEQUENCE']),
      habits: z.array(z.string()),
      reason: z.string(),
    })),
  });

  export async function analyzeHabitCorrelations(
    habits: Habit[],
    logs: HabitLog[]
  ) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `分析多个习惯之间的关联性。
寻找：
1. 正相关习惯（一起完成）
2. 负相关习惯（互相排斥）
3. 习惯堆叠的机会`,
      prompt: `习惯: ${JSON.stringify(habits)}
记录: ${JSON.stringify(logs)}`,
      schema: habitCorrelationSchema,
    });
    return object;
  }
  ```

  **4. 中断风险预测**

  ```typescript
  const breakRiskSchema = z.object({
    overallRisk: z.enum(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']),
    riskScore: z.number().min(0).max(100),
    riskFactors: z.array(z.object({
      factor: z.string(),
      weight: z.number(),
      currentStatus: z.string(),
    })),
    warningSignals: z.array(z.object({
      signal: z.string(),
      detected: z.boolean(),
      lastOccurrence: z.string().optional(),
    })),
    preventiveActions: z.array(z.object({
      action: z.string(),
      priority: z.enum(['HIGH', 'MEDIUM', 'LOW']),
      expectedImpact: z.string(),
    })),
  });

  export async function predictBreakRisk(
    habit: Habit,
    recentLogs: HabitLog[]
  ) {
    const { object } = await generateObject({
      model: 'openai/gpt-4o',
      system: `预测习惯中断风险。
风险信号包括：
1. 完成率下降趋势
2. 延迟完成增加
3. 主观难度评分上升
4. 情绪分数下降
5. 跳过天数增加`,
      prompt: `习惯: ${JSON.stringify(habit)}
近期记录: ${JSON.stringify(recentLogs)}`,
      schema: breakRiskSchema,
    });
    return object;
  }
  ```

  #### 可视化组件

  ```tsx
  // src/app/_components/analytics/time-heatmap.tsx
  'use client';

  import {
    ResponsiveContainer,
    ScatterChart,
    Scatter,
    XAxis,
    YAxis,
    Tooltip,
    Cell
  } from 'recharts';
  import { api } from '@/trpc/react';

  export function TimeHeatmap({ habitId }: { habitId?: string }) {
    const { data } = api.analytics.getTimeHeatmap.useQuery({ habitId });

    const getColor = (rate: number) => {
      if (rate >= 0.8) return '#22c55e'; // green
      if (rate >= 0.6) return '#84cc16'; // lime
      if (rate >= 0.4) return '#eab308'; // yellow
      if (rate >= 0.2) return '#f97316'; // orange
      return '#ef4444'; // red
    };

    return (
      <div className="h-64">
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 40 }}>
            <XAxis
              dataKey="hourOfDay"
              name="小时"
              domain={[0, 23]}
              tickFormatter={(h) => `${h}:00`}
            />
            <YAxis
              dataKey="dayOfWeek"
              name="星期"
              domain={[0, 6]}
              tickFormatter={(d) => ['日','一','二','三','四','五','六'][d]}
            />
            <Tooltip
              formatter={(value, name) => [
                name === 'completionRate' ? `${(value * 100).toFixed(0)}%` : value,
                name === 'completionRate' ? '完成率' : name
              ]}
            />
            <Scatter data={data?.heatmap} fill="#8884d8">
              {data?.heatmap.map((entry, index) => (
                <Cell key={index} fill={getColor(entry.completionRate)} />
              ))}
            </Scatter>
          </ScatterChart>
        </ResponsiveContainer>
      </div>
    );
  }
  ```

  #### 分析 tRPC Router

  ```typescript
  // src/server/api/routers/analytics.ts
  import { createTRPCRouter, protectedProcedure } from '@/server/api/trpc';
  import { z } from 'zod';
  import {
    analyzeTimePatterns,
    analyzeMoodCorrelation,
    analyzeHabitCorrelations,
    predictBreakRisk
  } from '@/lib/ai/analytics';

  export const analyticsRouter = createTRPCRouter({
    getTimeHeatmap: protectedProcedure
      .input(z.object({ habitId: z.string().optional() }))
      .query(async ({ ctx, input }) => {
        const logs = await ctx.db.habitLog.findMany({
          where: {
            userId: ctx.session.user.id,
            ...(input.habitId && { habitId: input.habitId }),
          },
          orderBy: { loggedAt: 'desc' },
          take: 90, // 最近90天
        });
        return analyzeTimePatterns(logs);
      }),

    getMoodAnalysis: protectedProcedure
      .input(z.object({ habitId: z.string() }))
      .query(async ({ ctx, input }) => {
        const logs = await ctx.db.habitLog.findMany({
          where: {
            userId: ctx.session.user.id,
            habitId: input.habitId,
            moodBefore: { not: null },
          },
        });
        return analyzeMoodCorrelation(logs);
      }),

    getHabitCorrelations: protectedProcedure
      .query(async ({ ctx }) => {
        const habits = await ctx.db.habit.findMany({
          where: { userId: ctx.session.user.id, status: 'ACTIVE' },
        });
        const logs = await ctx.db.habitLog.findMany({
          where: { userId: ctx.session.user.id },
          orderBy: { loggedAt: 'desc' },
          take: 180, // 最近180天
        });
        return analyzeHabitCorrelations(habits, logs);
      }),

    getBreakRiskPrediction: protectedProcedure
      .input(z.object({ habitId: z.string() }))
      .query(async ({ ctx, input }) => {
        const habit = await ctx.db.habit.findUnique({
          where: { id: input.habitId },
        });
        const logs = await ctx.db.habitLog.findMany({
          where: { habitId: input.habitId },
          orderBy: { loggedAt: 'desc' },
          take: 14, // 最近14天
        });
        return predictBreakRisk(habit!, logs);
      }),
  });
  ```

  ---

  ## 5. AI SDK 技术实现（v6 最新版）

  ### 5.1 Vercel AI Gateway 模型配置

  ```typescript
  // 直接在调用时指定模型，无需单独配置文件
  // 支持的模型格式：provider/model-name

  // streamText
  streamText({ model: 'openai/gpt-4o', ... });

  // generateObject
  generateObject({ model: 'openai/gpt-4o', ... });

  // generateText
  generateText({ model: 'anthropic/claude-sonnet-4-20250514', ... });
  ```

  ### 5.2 环境变量配置

  ```env
  # .env.local

  # Vercel AI Gateway（统一 API Key，访问多个提供商）
  AI_GATEWAY_API_KEY=your-api-key

  # 数据库 (Prisma)
  DATABASE_URL=postgresql://user:password@localhost:5432/micro_gravity

  # NextAuth.js v5
  AUTH_SECRET=your-auth-secret-here
  AUTH_DISCORD_ID=your-discord-client-id
  AUTH_DISCORD_SECRET=your-discord-client-secret
  ```

  ### 5.3 核心 API 使用模式

  #### streamText - 对话流式响应

  ```typescript
  import { streamText, convertToModelMessages, UIMessage } from 'ai';

  export async function POST(req: Request) {
    const { messages }: { messages: UIMessage[] } = await req.json();

    const result = streamText({
      model: 'openai/gpt-4o',
      system: SYSTEM_PROMPT,
      messages: convertToModelMessages(messages),
    });

    return result.toUIMessageStreamResponse();
  }
  ```

  #### generateObject - 结构化输出

  ```typescript
  import { generateObject } from 'ai';
  import { z } from 'zod';

  const schema = z.object({
    field1: z.string(),
    field2: z.number(),
  });

  const { object } = await generateObject({
    model: 'openai/gpt-4o',
    system: 'System prompt here',
    prompt: 'User prompt here',
    schema: schema,
  });
  ```

  #### tool - 工具定义

  ```typescript
  import { tool } from 'ai';
  import { z } from 'zod';

  const myTool = tool({
    description: '工具描述',
    inputSchema: z.object({
      param1: z.string().describe('参数描述'),
    }),
    execute: async ({ param1 }) => {
      // 执行逻辑
      return result;
    },
  });
  ```

  ### 5.4 多步骤调用（Agent 模式）

  ```typescript
  import { streamText, stepCountIs, convertToModelMessages } from 'ai';

  const result = streamText({
    model: 'openai/gpt-4o',
    messages: convertToModelMessages(messages),
    tools: { /* ... */ },
    stopWhen: stepCountIs(5), // 最多5步
  });
  ```

  ### 5.5 前端 useChat Hook

  ```typescript
  'use client';
  import { useChat } from '@ai-sdk/react';
  import { DefaultChatTransport } from 'ai';
  import { useState } from 'react';

  export function ChatComponent() {
    const [input, setInput] = useState('');
    const { messages, sendMessage, isLoading } = useChat({
      transport: new DefaultChatTransport({
        api: '/api/chat',
      }),
    });

    // 发送消息
    const handleSubmit = () => {
      sendMessage({ text: input });
      setInput('');
    };

    // 渲染消息（使用 parts）
    return (
      <div>
        {messages.map(m => (
          <div key={m.id}>
            {m.parts.map((part, i) => {
              if (part.type === 'text') return <p key={i}>{part.text}</p>;
              if (part.type === 'tool-invocation') {
                return <div key={i}>工具调用: {part.toolName}</div>;
              }
              return null;
            })}
          </div>
        ))}
      </div>
    );
  }
  ```

  ---

  ## 6. 数据库设计

  ### 6.1 Schema 定义（Prisma ORM）

  ```prisma
  // prisma/schema.prisma

  generator client {
    provider = "prisma-client-js"
    output   = "../generated/prisma"
  }

  datasource db {
    provider = "postgresql"
    url      = env("DATABASE_URL")
  }

  // NextAuth.js 所需模型
  model Account {
    id                       String  @id @default(cuid())
    userId                   String
    type                     String
    provider                 String
    providerAccountId        String
    refresh_token            String?
    access_token             String?
    expires_at               Int?
    token_type               String?
    scope                    String?
    id_token                 String?
    session_state            String?
    user                     User    @relation(fields: [userId], references: [id], onDelete: Cascade)
    @@unique([provider, providerAccountId])
  }

  model Session {
    id           String   @id @default(cuid())
    sessionToken String   @unique
    userId       String
    expires      DateTime
    user         User     @relation(fields: [userId], references: [id], onDelete: Cascade)
  }

  model User {
    id                 String    @id @default(cuid())
    name               String?
    email              String?   @unique
    emailVerified      DateTime?
    image              String?
    motivationProfile  Json?     // 动机画像
    preferences        Json?
    accounts           Account[]
    sessions           Session[]
    habits             Habit[]
    habitLogs          HabitLog[]
    conversations      Conversation[]
    // v1.5 新增
    reports            Report[]
    milestones         Milestone[]
    analyticsSnapshots AnalyticsSnapshot[]
    createdAt          DateTime  @default(now())
    updatedAt          DateTime  @updatedAt
  }

  model VerificationToken {
    identifier String
    token      String   @unique
    expires    DateTime
    @@unique([identifier, token])
  }

  // 习惯类型枚举
  enum HabitType {
    BUILD  // 养成
    BREAK  // 戒除
  }

  enum HabitStatus {
    ACTIVE
    PAUSED
    COMPLETED
    ARCHIVED
  }

  // 对话类型枚举
  enum ConversationType {
    SETUP      // 习惯设定
    CHECKIN    // 日常签到
    HELP       // 困难求助
    REVIEW     // 复盘分析
  }

  // 分析类型枚举
  enum AnalysisType {
    TIME_HEATMAP   // 时间热力图
    MOOD           // 情绪分析
    CORRELATION    // 习惯关联
    RISK           // 中断风险
  }

  model Habit {
    id           String      @id @default(cuid())
    userId       String
    user         User        @relation(fields: [userId], references: [id], onDelete: Cascade)
    type         HabitType
    name         String      @db.VarChar(100)
    description  String?
    category     String?     @db.VarChar(50)

    // MAP模型数据 (福格行为模型) - 参见 6.2 类型定义
    motivation   Json        // Motivation: { primaryType, deepReason, visionStatement, painPoints?, motivationScore }
    ability      Json        // Ability: { currentLevel, targetLevel, microHabit, difficultyScore, barriers?, simplificationTips? }
    prompt       Json        // Prompt: { anchorHabit, triggerType, preferredTime, contextCues?, reminderStyle? }

    // 进度追踪
    currentPhase Int         @default(1)
    phases       Json?       // PhaseConfig[]: 渐进式阶段配置

    status       HabitStatus @default(ACTIVE)
    logs         HabitLog[]
    conversations Conversation[]
    // v1.5 新增
    milestones   Milestone[]
    createdAt    DateTime    @default(now())
    updatedAt    DateTime    @updatedAt

    @@index([userId])
  }

  model HabitLog {
    id               String    @id @default(cuid())
    habitId          String
    habit            Habit     @relation(fields: [habitId], references: [id], onDelete: Cascade)
    userId           String
    user             User      @relation(fields: [userId], references: [id], onDelete: Cascade)
    loggedAt         DateTime  @db.Date
    completed        Boolean

    completionTime   DateTime?
    durationMinutes  Int?
    difficultyRating Int?      // 1-5
    moodBefore       Int?      // 1-5
    moodAfter        Int?      // 1-5
    notes            String?
    triggerContext   Json?     // TriggerRecord: 戒除场景记录 (仅 BREAK 类型习惯)

    createdAt        DateTime  @default(now())

    @@index([habitId])
    @@index([userId])
    @@index([loggedAt])
    @@unique([habitId, loggedAt]) // 防止同一天重复打卡
  }

  model Conversation {
    id               String            @id @default(cuid())
    userId           String
    user             User              @relation(fields: [userId], references: [id], onDelete: Cascade)
    habitId          String?
    habit            Habit?            @relation(fields: [habitId], references: [id], onDelete: SetNull)
    conversationType ConversationType? // 使用枚举
    messages         Json              // AI对话历史
    createdAt        DateTime          @default(now())
    updatedAt        DateTime          @updatedAt

    @@index([userId])
    @@index([habitId])
  }

  // ============ v1.5 新增模型 ============

  // 报告类型枚举
  enum ReportType {
    WEEKLY      // 周报
    MONTHLY     // 月报
    MILESTONE   // 里程碑报告
  }

  // 里程碑类型
  enum MilestoneType {
    DAY_7       // 7天成就
    DAY_21      // 21天习惯初步形成
    DAY_66      // 66天习惯巩固
    DAY_100     // 100天里程碑
    CUSTOM      // 自定义里程碑
  }

  model Report {
    id           String       @id @default(cuid())
    userId       String
    user         User         @relation(fields: [userId], references: [id], onDelete: Cascade)
    type         ReportType

    // 报告周期
    periodStart  DateTime
    periodEnd    DateTime

    // AI 生成的报告内容
    summary      Json         // { completionRate, rateChange, activeHabits, ... }
    highlights   Json         // 亮点数组
    patterns     Json         // 模式发现数组
    suggestions  Json         // 建议数组
    goals        Json?        // 下期目标

    // 元数据
    generatedAt  DateTime     @default(now())
    isRead       Boolean      @default(false)

    @@index([userId])
    @@index([type])
    @@index([periodStart])
    @@unique([userId, type, periodStart]) // 防止同周期重复生成
  }

  model Milestone {
    id           String        @id @default(cuid())
    habitId      String
    habit        Habit         @relation(fields: [habitId], references: [id], onDelete: Cascade)
    userId       String
    user         User          @relation(fields: [userId], references: [id], onDelete: Cascade)
    type         MilestoneType

    // 达成信息
    achievedAt   DateTime
    streakDays   Int           // 达成时的连续天数

    // AI 生成的里程碑报告
    celebration  String        // 庆祝文案
    reflection   Json          // 回顾分析
    nextPhase    Json?         // 下阶段建议

    // 分享相关
    isShared     Boolean       @default(false)
    shareToken   String?       @unique

    createdAt    DateTime      @default(now())

    @@index([habitId])
    @@index([userId])
    @@index([type])
    @@unique([habitId, type]) // 同一习惯同一里程碑类型只能达成一次
  }

  // 分析快照 - 缓存 AI 分析结果
  model AnalyticsSnapshot {
    id           String       @id @default(cuid())
    userId       String
    user         User         @relation(fields: [userId], references: [id], onDelete: Cascade)
    habitId      String?      // 可选，null 表示全局分析

    // 分析类型
    analysisType AnalysisType // 使用枚举

    // 缓存的分析结果
    data         Json

    // 有效期
    generatedAt  DateTime     @default(now())
    expiresAt    DateTime

    @@index([userId])
    @@index([analysisType])
    @@unique([userId, habitId, analysisType])
  }
  ```

  > **v1.5 数据模型说明**:
  > - `Report`: 存储 AI 生成的周报/月报，支持历史查看
  > - `Milestone`: 记录习惯里程碑成就，支持社交分享
  > - `AnalyticsSnapshot`: 缓存 AI 分析结果，避免重复计算

  ### 6.2 类型定义

  ```typescript
  // lib/types.ts
  import { z } from 'zod';

  // 动机配置
  export const motivationSchema = z.object({
    primaryType: z.enum(['PLEASURE', 'HOPE', 'SOCIAL']),
    deepReason: z.string(),
    visionStatement: z.string(),
    painPoints: z.array(z.string()).optional(),
    motivationScore: z.number().min(1).max(10),
  });

  // 能力配置
  export const abilitySchema = z.object({
    currentLevel: z.string(),
    targetLevel: z.string(),
    microHabit: z.string(),
    difficultyScore: z.number().min(1).max(10),
    barriers: z.array(z.string()).optional(),
    simplificationTips: z.array(z.string()).optional(),
  });

  // 提示配置
  export const promptSchema = z.object({
    anchorHabit: z.string(),
    triggerType: z.enum(['SIGNAL', 'FACILITATOR', 'SPARK']),
    preferredTime: z.string(),
    contextCues: z.array(z.string()).optional(),
    reminderStyle: z.enum(['GENTLE', 'FIRM', 'PLAYFUL']).optional(),
  });

  // 完整习惯配置
  export const habitConfigSchema = z.object({
    name: z.string(),
    type: z.enum(['BUILD', 'BREAK']),
    category: z.string().optional(),
    motivation: motivationSchema,
    ability: abilitySchema,
    prompt: promptSchema,
  });

  export type HabitConfig = z.infer<typeof habitConfigSchema>;
  export type Motivation = z.infer<typeof motivationSchema>;
  export type Ability = z.infer<typeof abilitySchema>;
  export type Prompt = z.infer<typeof promptSchema>;

  // ============ 上下文类型定义 ============

  // 提醒生成上下文
  export const reminderContextSchema = z.object({
    habitId: z.string(),
    habitName: z.string(),
    habitType: z.enum(['BUILD', 'BREAK']),
    motivationType: z.enum(['PLEASURE', 'HOPE', 'SOCIAL']),
    motivationLevel: z.number().min(1).max(10),
    streakDays: z.number(),
    recentRate: z.number().min(0).max(100), // 最近7天完成率
    lastCompletedAt: z.date().optional(),
    currentPhase: z.number(),
  });
  export type ReminderContext = z.infer<typeof reminderContextSchema>;

  // 动机激励上下文
  export const motivationContextSchema = z.object({
    habitId: z.string(),
    habitName: z.string(),
    deepReason: z.string(),
    visionStatement: z.string(),
    streakDays: z.number(),
    currentState: z.enum(['STRONG', 'NORMAL', 'DECLINING', 'CRITICAL']),
    daysSinceStart: z.number(),
    recentCompletionRate: z.number(),
  });
  export type MotivationContext = z.infer<typeof motivationContextSchema>;

  // 数据洞察上下文
  export const habitDataSchema = z.object({
    habit: z.object({
      id: z.string(),
      name: z.string(),
      type: z.enum(['BUILD', 'BREAK']),
      currentPhase: z.number(),
      createdAt: z.date(),
    }),
    logs: z.array(z.object({
      loggedAt: z.date(),
      completed: z.boolean(),
      difficultyRating: z.number().optional(),
      moodBefore: z.number().optional(),
      moodAfter: z.number().optional(),
    })),
    stats: z.object({
      totalDays: z.number(),
      completedDays: z.number(),
      currentStreak: z.number(),
      longestStreak: z.number(),
      averageDifficulty: z.number().optional(),
    }),
  });
  export type HabitData = z.infer<typeof habitDataSchema>;

  // 触发记录 (坏习惯戒除)
  export const triggerRecordSchema = z.object({
    timestamp: z.date(),
    triggerType: z.enum(['TEMPORAL', 'CONTEXTUAL', 'EMOTIONAL', 'BEHAVIORAL']),
    context: z.string(), // 触发时的场景描述
    location: z.string().optional(),
    emotion: z.string().optional(),
    intensity: z.number().min(1).max(10), // 冲动强度
    resisted: z.boolean(), // 是否成功抵抗
    copingStrategy: z.string().optional(), // 使用的应对策略
  });
  export type TriggerRecord = z.infer<typeof triggerRecordSchema>;

  // ============ v1.5 报告类型定义 ============

  // 周报摘要
  export const reportSummarySchema = z.object({
    completionRate: z.number(),
    rateChange: z.number(), // 较上周变化
    activeHabits: z.number(),
    longestStreak: z.number(),
    totalCheckins: z.number(),
    perfectDays: z.number(), // 全部完成的天数
  });
  export type ReportSummary = z.infer<typeof reportSummarySchema>;

  // 报告亮点
  export const reportHighlightSchema = z.object({
    habitId: z.string(),
    habitName: z.string(),
    achievement: z.string(),
    emoji: z.string(),
    metric: z.string().optional(), // 具体数据，如 "连续7天"
  });
  export type ReportHighlight = z.infer<typeof reportHighlightSchema>;

  // 模式发现
  export const patternFindingSchema = z.object({
    finding: z.string(),
    implication: z.string(),
    confidence: z.number().min(0).max(1),
    dataPoints: z.number(), // 支撑数据点数量
  });
  export type PatternFinding = z.infer<typeof patternFindingSchema>;

  // 里程碑回顾
  export const milestoneReflectionSchema = z.object({
    journey: z.string(), // 历程总结
    keyMoments: z.array(z.string()), // 关键时刻
    lessonsLearned: z.array(z.string()), // 学到的经验
    strengthsShown: z.array(z.string()), // 展现的优势
  });
  export type MilestoneReflection = z.infer<typeof milestoneReflectionSchema>;

  // 渐进式阶段配置
  export const phaseConfigSchema = z.object({
    phase: z.number(),
    name: z.string(),
    duration: z.string(), // 如 "7天"
    microHabit: z.string(),
    successCriteria: z.string(),
    difficultyScore: z.number().min(1).max(10),
    startedAt: z.date().optional(),
    completedAt: z.date().optional(),
  });
  export type PhaseConfig = z.infer<typeof phaseConfigSchema>;
  ```

  ---

  ## 7. 非功能需求

  ### 7.1 性能要求

  | 指标 | 目标值 |
  |-----|-------|
  | 页面加载 | < 2s |
  | AI 对话响应 | < 3s（首 token） |
  | API 响应 | < 200ms（非 AI） |
  | 推送延迟 | < 30s |

  ### 7.2 安全要求

  - NextAuth.js v5 认证 (Discord OAuth)
  - Session-based 认证 (NextAuth 默认)
  - tRPC protectedProcedure 保护敏感端点
  - API Rate Limiting
  - 敏感数据加密存储
  - GDPR 数据删除权

  ### 7.3 监控

  - Vercel Analytics（性能）
  - Sentry（错误追踪）
  - PostHog（用户行为）

  ---

  ## 8. 版本迭代规划

  ### 8.1 版本路线总览

  ```
  v1.0 (MVP) ──────────────────────────────────────────────────── 基础版
  ├── AI 对话式习惯创建 (模块一)
  ├── 每日打卡记录
  ├── 智能提醒系统 (模块二)
  └── 基础数据看板 (模块七)

  v1.5 (增强) ◀── 当前开发目标 ─────────────────────────────────── 增强版
  ├── 坏习惯戒除专项 (模块五)
  ├── 动机维护系统 (模块三)
  ├── 周期报告系统 (模块八) ⭐ 新增
  └── 进阶分析系统 (模块九) ⭐ 新增

  v2.0 (社交) ─────────────────────────────────────────────────── 社交版
  ├── 习惯社区
  ├── 问责伙伴
  ├── 成就分享
  └── 排行榜

  v2.5 (智能) ─────────────────────────────────────────────────── 智能版
  ├── 行为预测
  ├── 跨习惯关联
  ├── 语音交互
  └── 智能手表集成
  ```

  ### 8.2 v1.5 详细功能清单

  #### 8.2.1 坏习惯戒除专项 (模块五)

  | 功能点 | 描述 | 优先级 |
  |-------|------|-------|
  | 触发模式识别 | AI 分析坏习惯的时间/情境/情绪触发模式 | P0 |
  | 替代行为设计 | 根据深层需求推荐替代行为 | P0 |
  | 环境设计建议 | 提供增加执行障碍的具体建议 | P1 |
  | 复发管理 | 共情响应 + 模式分析 + 策略迭代 | P0 |
  | 触发链打断 | 可视化触发链并提供打断策略 | P1 |

  #### 8.2.2 动机维护系统 (模块三)

  | 功能点 | 描述 | 优先级 |
  |-------|------|-------|
  | 动机波动检测 | 基于打卡数据识别动机下降趋势 | P0 |
  | 关键干预点 | Day 3/7/14/21 自动触发激励 | P0 |
  | 个性化激励 | 根据用户动机类型生成定制文案 | P0 |
  | 愿景可视化 | 定期回顾用户设定的愿景声明 | P1 |
  | 损失提醒 | 计算放弃的机会成本 | P2 |

  #### 8.2.3 周期报告系统 (模块八) ⭐

  | 功能点 | 描述 | 优先级 |
  |-------|------|-------|
  | 周报生成 | 每周日自动生成 AI 周报 | P0 |
  | 月报生成 | 每月末自动生成月度报告 | P0 |
  | 里程碑报告 | 7/21/66天自动生成成就报告 | P0 |
  | 报告历史 | 查看历史报告，对比进步 | P1 |
  | 报告分享 | 生成分享卡片到社交媒体 | P2 |
  | 报告订阅 | 邮件/推送订阅周报 | P2 |

  #### 8.2.4 进阶分析系统 (模块九) ⭐

  | 功能点 | 描述 | 优先级 |
  |-------|------|-------|
  | 时间热力图 | 可视化最佳执行时间窗口 | P0 |
  | 情绪关联分析 | 分析习惯与情绪的双向关系 | P1 |
  | 习惯关联矩阵 | 发现习惯间的正/负相关关系 | P1 |
  | 中断风险预测 | 提前预警习惯中断风险 | P0 |
  | 难度曲线图 | 追踪主观难度随时间的变化 | P2 |
  | 分析缓存 | 智能缓存分析结果，提升性能 | P1 |

  ### 8.3 v1.5 技术实现要点

  ```typescript
  // 新增 tRPC Routers (需注册到 appRouter)
  src/server/api/routers/
  ├── report.ts      // 周期报告
  ├── analytics.ts   // 进阶分析
  └── milestone.ts   // 里程碑管理

  // 新增 AI 功能模块
  src/lib/ai/
  ├── analytics.ts   // 分析函数集合
  ├── report.ts      // 报告生成函数
  └── motivation.ts  // 动机维护函数

  // 新增前端组件
  src/app/_components/
  ├── analytics/
  │   ├── time-heatmap.tsx
  │   ├── mood-chart.tsx
  │   ├── correlation-matrix.tsx
  │   └── risk-indicator.tsx
  ├── reports/
  │   ├── weekly-report.tsx
  │   ├── monthly-report.tsx
  │   └── milestone-card.tsx
  └── break-habit/
      ├── trigger-analysis.tsx
      └── relapse-form.tsx

  // Vercel Cron Jobs
  vercel.json → crons 配置
  src/app/api/cron/
  ├── weekly-report/route.ts
  └── monthly-report/route.ts
  ```

  ### 8.4 v1.5 数据库变更

  ```prisma
  // 新增枚举
  enum ReportType { WEEKLY, MONTHLY, MILESTONE }
  enum MilestoneType { DAY_7, DAY_21, DAY_66, DAY_100, CUSTOM }

  // 新增模型
  model Report { ... }           // 周期报告存储
  model Milestone { ... }        // 里程碑记录
  model AnalyticsSnapshot { ... } // 分析结果缓存
  ```

  ### 8.5 v2.0+ 扩展功能

  | 功能 | 描述 | 版本 |
  |-----|------|------|
  | 习惯社区 | 用户分享习惯心得和技巧 | v2.0 |
  | 问责伙伴 | 匹配互相监督的伙伴 | v2.0 |
  | 排行榜 | 连续天数/完成率排名 | v2.0 |
  | 行为预测 | 预测明日完成概率 | v2.5 |
  | 语音交互 | Web Speech API 语音打卡 | v2.5 |
  | 可穿戴设备 | Apple Watch/Wear OS 集成 | v2.5 |
  | 家庭/团队版 | 多人协作习惯养成 | v2.5 |
  | API 开放 | 第三方应用集成 | v3.0 |

  ---

  ## 附录

  ### A. 术语表

  | 术语 | 定义 |
  |-----|------|
  | 福格行为模型 | BJ Fogg 提出，B=MAP |
  | 微习惯 | 极其简单的目标行为 |
  | 锚定习惯 | 新习惯附着的已有习惯 |
  | 触发点 | 引发行为的信号 |

  ### B. 参考资料

  - Fogg, BJ. "Tiny Habits"
  - Clear, James. "Atomic Habits"
  - [Vercel AI SDK Documentation](https://v6.ai-sdk.dev/llms.txt)
  - [Next.js Documentation](https://nextjs.org/docs)

  ---

  **文档版本**: v2.4 (AI SDK v6 升级版)
  **更新日期**: 2025-11-28
  **技术框架**: Next.js 15 + tRPC v11 + Prisma + NextAuth.js v5 + Vercel AI SDK v6
  **当前开发目标**: v1.5 增强版
